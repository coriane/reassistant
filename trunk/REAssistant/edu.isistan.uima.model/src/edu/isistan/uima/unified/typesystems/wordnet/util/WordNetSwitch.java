 ...
 * $Id$
 */
package edu.isistan.uima.unified.typesystems.wordnet.util;
...
public class WordNetSwitch<T> {
	...
	protected T doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			case WordNetPackage.SENSE: {
				Sense sense = (Sense)theEObject;
				T result = caseSense(sense);
				if (result == null) result = caseIdentifiableAnnotation(sense);
				if (result == null) result = caseAnnotation(sense);
				if (result == null) result = caseAnnotationBase(sense);
				if (result == null) result = caseTOP(sense);
